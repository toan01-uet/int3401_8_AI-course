        ...
        for iteration in range(self.iterations):
            ...
            for state in states:
                ...
                for action in actions:
                    transitionStatesProbs = self.mdp.getTransitionStatesAndProbs(
                        state, action)
                    sumvalue = 0.0
                    for stateProb in transitionStatesProbs:
                        sumvalue += stateProb[1] * (self.mdp.getReward(
                            state, action, stateProb[0]) + self.discount * self.values[stateProb[0]])
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for stateProb in transitionStatesProbs:
            value += stateProb[1] * (self.mdp.getReward(state, action,
                                                        stateProb[0]) + self.discount * self.values[stateProb[0]])
        return value
